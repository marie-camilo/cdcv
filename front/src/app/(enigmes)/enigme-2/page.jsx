...
export default function Enigme2Page() {
    ...
    // Vérifier si victoire (bon casier trouvé + les 4 chiffres)
    useEffect(() => {
        const hasAllDigits = Object.values(codeDigits).every(v => v !== null);

        // Vérifier si le bon casier a au moins une case "success"
        ...
        if (hasAllDigits && hasFoundCorrectLocker) {
            // Marquer le bon casier comme validé
            if (CORRECT_LOCKER === 'left') {
                setLeftLocker('validated');
            } else {
                setRightLocker('validated');
            }
    ...
    // Gérer la confirmation de victoire
        ...
        <main className="min-h-screen flex flex-col md:max-w-md mx-auto p-4">
            ...
            {showVictory && (
                <div className="fixed inset-0 bg-black/95 flex items-center justify-center z-50 p-4">
                    <div className="bg-gray-900 border-4 border-green-500 rounded-lg p-8 max-w-md text-center animate-pulse-slow">
                        <h2 className="text-3xl font-bold text-green-400 mb-4">ACCÈS AUTORISÉ</h2>
                        <p className="text-white mb-4">
                            Excellent travail ! Vous avez déchiffré le système et trouvé le bon casier <span className="font-bold text-green-400">{CORRECT_LOCKER === 'left' ? 'GAUCHE' : 'DROITE'}</span>.
                        </p>
                        <p className="text-white text-sm mb-6">
                            Code du casier : <span className="text-2xl font-mono font-bold text-green-400">{Object.values(codeDigits).join('')}</span>
                        </p>
                        <button
                            onClick={handleVictoryConfirm}
                            className="bg-green-500 hover:bg-green-600 text-white font-bold py-3 px-8 rounded-lg transition-all text-lg"
                        >
                            CONTINUER LA MISSION
                        </button>
                        <p className="text-gray-400 italic text-xs mt-4">
                            "Le temps presse... Dépêchez-vous ! ⏱️"
                        </p>
                    </div>
                </div>
            )}
