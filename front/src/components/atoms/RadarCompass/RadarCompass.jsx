    ...
    if (!permissionGranted) {
        return (
            <div className="flex flex-col items-center justify-center w-64 h-64 border border-[var(--color-light-green)] rounded-full bg-black/90 p-4 text-center shadow-[0_0_30px_rgba(0,255,0,0.1)] z-50">
                <p className="text-[var(--color-light-green)] text-[10px] mb-4 font-mono tracking-widest uppercase font-black">System Offline</p>
                <button onClick={requestAccess} className="px-6 py-2 border border-[var(--color-light-green)] text-[var(--color-light-green)] font-mono text-xs hover:bg-[var(--color-light-green)] hover:text-black transition-all">INITIALISER</button>
            </div>
        );
    }
    ...
    if (isSearching) {
        return (
            <div className="relative w-64 h-64 rounded-full border border-[var(--color-light-green)] bg-black/90 flex items-center justify-center overflow-hidden z-20">
                ...
                <div className="z-10 text-[var(--color-light-green)] font-mono text-xs animate-pulse tracking-widest uppercase font-black">Calibration...</div>
            </div>
        );
    }
        ...
        <div className="relative w-72 h-72 select-none z-20 flex items-center justify-center">
            ...
            >
                ...
                {showDots && targets.map((target) => {
                    ...
                    const color = isSaboteurType ? "var(--color-lavender)" : "var(--color-classic-red)";
            ...
            {/* Cap Digital */}
            <div className="absolute -bottom-8 left-1/2 -translate-x-1/2 text-[var(--color-light-green)] font-mono text-xs font-bold tracking-[2px] opacity-80 border-x border-[var(--color-light-green)]/30 px-3">
